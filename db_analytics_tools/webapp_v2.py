...
class DBAnalyticsUI:
    ...
    def __init__(self, config):
        ...
        self.app_description = config.get("app_description", "")
    ...
    def run(self):
        ...
        """
        st.set_page_config(page_title=self.app_name, page_icon=self.app_favicon, layout="wide")
    ...
    def db_page_sessions(self):
        ...
        try:
            ...
            #     if st.button("Terminate PID", type="secondary", width='stretch'):
            #         client.cancel_query(pid_to_kill)
            #         st.success(f"Signal envoyé au PID {pid_to_kill}")
            with col1:
                st.subheader("Terminate session (PID)")
                with st.form("kill_pid_form", clear_on_submit=False, border=False):
                    ...
                    if submit_kill:
                        client.cancel_query(pid_to_kill)
                        st.success(f"Signal sent to PID {pid_to_kill}")
    ...
    def db_page_upload_download(self):
        ...
        with col1:
            ...
            if file and file.name.endswith(".csv"):
                ...
                with colsep:
                    separator = st.selectbox("Delimiter", options=[",", ";", "\t", "|"])
                with coldec:
                    decimal = st.selectbox("CSV Delimiter", options=[".", ","])
                file_ok = True
...
import streamlit as st
import json
from db_analytics_tools.webapp_v2 import DBAnalyticsUI
